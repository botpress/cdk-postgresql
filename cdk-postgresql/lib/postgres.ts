...
export const createRole = async (props: { client: Client; name: string; password: string }) => {
  const { client, name, password } = props

  await client.query(format('CREATE USER %I WITH PASSWORD %L', name, password))
}

export const createDatabase = async (props: { client: Client; name: string; owner: string }) => {
  ...
  try {
    ...
    if (
      !isDatabaseError(e) ||
      !(e.code === '0LP01' && e.message === `role "${owner}" is a member of role "${client.user}"`)
    ) {
