...
const getPassword = async (connection: Connection) => {
  ...
  if (isObject(parsedSecret)) {
    if (!connection.PasswordField) {
      throw new Error('connection.PasswordField must be specified if secret is object')
    }
    if (!parsedSecret[connection.PasswordField]) {
      throw new Error(`PasswordField ${connection.PasswordField} was not found in secret`)
    }
